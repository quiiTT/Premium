window.addEventListener("message", async e => {
  ...
  // Target for testing the player ONLY on localhost
  ...
  console.log('[quiT] Player has been found.')
  ...
  for (let stream of streamlist) {
    // Premium - Versions "International Dub"
    if (stream.format == 'trailer_hls' && stream.hardsub_lang == user_lang || (streamlist.length < 15 && stream.hardsub_lang === null))
      if (rows_number <= 4) {
        ...
        rows_number++;
        // mp4 + temporary resolve until you get direct link from m3u8.
        if (rows_number > 4) {
  ...
  // Checks if the URL of video_mp4_array[id] exists and calculates the download size.
  function linkDownload(id, tentativas=0) {
    console.log('  Currently downloading: ', r[id])
    ...
    http.onreadystatechange = () => {
      if (http.readyState == 4 && http.status == 200) {
        ...
        if (!fileSize)
          ...
          return console.log(`[quiT] Source has been added: ${r[id]} (${return_fileSize})`);
        }
  ...
  // Load player as soon as you find the m3u8 URLs.
  ...
  function startPlayer() {
    ...
    let update_tooltipText = "Updates available";
    ...
    // Function for closing download button.
    ...
    // Function for download per click.
    function download_ButtonClickAction() {
      ...
      if (!didDownload) {
        didDownload = true;
        console.log('[quiT] Downloading the sauce:')
    ...
    // Simple error due to no subtitles.
    jwplayer().on('error', e => {
      console.log(e)
      codes = { 232011: "https://i.gifer.com/T8kd.gif" };
      if (codes[e.code]) {
        jwplayer().load({
          file: codes[e.code]
        });
    ...
    // Keep saving the videos every 7 sec.
    setInterval(() => {
  ...
  async function m3u8ListFromStream(url) {
    ...
    if (master_m3u8) {
      ...
      m3u8list = streams.filter((el, idx) => idx%2===0) // %2 === 0 due to dns from ankai and cloudfare.
    } else {
      for (let i in r) {
        const idx = i;
